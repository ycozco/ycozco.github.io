---
title: Decolumax Jardinería
layout: page
permalink: /projects/decolumax/
---

<div class="project-page">
<div class="eyebrow">Web empresarial · SEO · Frontend · Deployment</div>

# Decolumax Jardinería

<p class="lead">Sitio web empresarial para una marca de jardinería y paisajismo, desarrollado con Astro y orientado a rendimiento, SEO técnico, responsive design y despliegue reproducible.</p>

<div class="project-facts">
  <div class="fact"><strong>Tipo</strong>Sitio web empresarial</div>
  <div class="fact"><strong>Enfoque</strong>Frontend, SEO y despliegue</div>
  <div class="fact"><strong>Estado</strong>Desplegado en VPS</div>
</div>

## Funcionalidades y características

- Diseño responsive para desktop y mobile.
- Componentización con Astro.
- Modo claro/oscuro con persistencia local.
- SEO técnico con canonical, robots y sitemap.
- Optimización de recursos con Sharp y SVGO.
- Build estático y servicio mediante Nginx.
- Docker Compose para entornos de build, desarrollo y producción.
- Despliegue en VPS con proxy reverso y SSL.

## Arquitectura de publicación

```text
Código Astro
    │
    ▼
Build estático
    │
    ▼
Contenedor Nginx
    │
    ▼
Nginx Proxy Manager / SSL
    │
    ▼
Sitio público
```

## Tecnologías

<div class="tech-list">
<span>Astro</span><span>Tailwind CSS</span><span>JavaScript</span><span>Sharp</span><span>SVGO</span><span>Docker</span><span>Nginx</span><span>SEO</span>
</div>

## Valor dentro del portafolio

Este proyecto complementa los sistemas más complejos del portafolio mostrando capacidad para entregar una solución web empresarial completa: diseño, implementación, optimización, SEO y puesta en producción.

<div class="callout"><strong>Repositorio:</strong> privado. <a target="_blank" href="https://decolumax-jardineria.qpsecuresolutions.cloud/">Ver sitio desplegado →</a></div>

</div>
